  ...
  <UCard class="w-[450px]">
    ...
    <UForm :state="state" :validate="validate" @submit="onLogin">
      ...
      <UiFlex justify="between" class="mt-4">
        <UButton color="gray" :disabled="!!loading" @click="navigateTo('/login/admin')">Quản trị viên</UButton>
...
const onLogin = async () => {
  try {
    loading.value = true
    await useAPI($API.Auth.Staff.Login, JSON.parse(JSON.stringify(state.value)))
