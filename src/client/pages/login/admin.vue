<template>
  <UCard class="w-[450px]">
    <template #header>
      ...
      <UiText size="sm" color="gray">Đăng nhập quản trị viên</UiText>
    </template>
    
    <UForm :state="state" :validate="validate" @submit="onLogin">
      <UFormGroup label="Tài khoản" name="username">
        <UInput icon="i-bxs-user" type="text" v-model="state.username" :disabled="!!loading" readonly />
      </UFormGroup>
      ...
      <UiFlex justify="between" class="mt-4">
        <UButton color="gray" :disabled="!!loading" @click="navigateTo('/login')">Nhân viên</UButton>
...
const onLogin = async () => {
  try {
    loading.value = true
    await useAPI($API.Auth.Admin.Login, JSON.parse(JSON.stringify(state.value)))
