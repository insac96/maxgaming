...
<template>
  <div>
    <StaffReceiveStatistic :statistic="statistic" class="mb-4"></StaffReceiveStatistic>
    ...
    <UCard :ui="{ 
      ...
      <template #header>
        <UiFlex>
          ...
          <UButton color="gray" @click="modal.add = true">Thêm khoản thu</UButton>
        </UiFlex>
    ...
    <UModal v-model="modal.add" preventClose>
      <UForm :state="stateAdd" @submit="addAction" class="p-4">
        ...
        <UFormGroup label="Ghi chú">
          <UTextarea v-model="stateAdd.note" />
        </UFormGroup>
...
const getList = async () => {
  try {
    ...
    const data = await useAPI($API.Receive.Staff.GetOnWorkShift, { ...page.value })
...
const addAction = async () => {
  try {
    ...
    await useAPI($API.Receive.Staff.Add, JSON.parse(JSON.stringify(stateAdd.value)))
