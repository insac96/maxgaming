...
<template>
  <div>
    <StaffSpendStatistic :statistic="statistic" class="mb-4"></StaffSpendStatistic>
...
const getList = async () => {
  try {
    ...
    const data = await useAPI($API.Spend.Staff.GetOnWorkShift, { ...page.value })
...
const addAction = async () => {
  try {
    ...
    await useAPI($API.Spend.Staff.Add, JSON.parse(JSON.stringify(stateAdd.value)))
